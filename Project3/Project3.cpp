...
void processCheck(double &, double &, double, bool &);
...
int main()
{
	...
	bool belowval = true;
	char option;
	double initbal, totalservice, servicefees;
	...
	while(flag)
	{
		...
		switch(option)
		{
			//process a check
			case 'C':
				processCheck(initbal, totalservice, servicefees, belowval);
				break;
...
void processCheck(double & bankval, double & totalservice, double servicefees, bool & belowval)
{
	...
	if(bankval < 500)
	{
		...
		{
			servicefees += 15;
			cout << "Service charge: $15.00 balance below $500.00" << endl;
			belowval = false;
		}
		else if(bankval < 0)
		{
	...
	totalservice += servicecharge + servicefees;
